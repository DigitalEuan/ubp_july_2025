...
class UBPResultsAnalyzer:
    ...
    def create_dataframe(self):
        ...
        for result in self.results:
            row = {
                ...
                'ubp_valid': result['validation']['ubp_signature_valid'],
                'pi_invariant': result['validation']['pi_invariant_achieved'],
                'pi_ratio_good': result['validation']['pi_ratio_good']
            }
            data.append(row)
    ...
    def create_visualizations(self):
        ...
        axes[0, 0].scatter(df['input_n'], df['s_pi_enhanced'], alpha=0.7, color='blue', label='S_π Enhanced')
        axes[0, 0].axhline(y=np.pi, color='red', linestyle='--', label='π target')
        ...
        # 2. S_π/π ratio
        axes[0, 1].scatter(df['input_n'], df['s_pi_ratio']*100, alpha=0.7, color='green')
        axes[0, 1].axhline(y=100, color='red', linestyle='--', label='100% target')
        ...
        axes[0, 2].scatter(df['sequence_length'], df['nrci_enhanced'], alpha=0.7, color='purple')
        ...
        axes[1, 0].scatter(df['input_n'], glyph_ratio, alpha=0.7, color='orange')
        ...
        axes[1, 1].hist(df['s_pi_error'], bins=10, alpha=0.7, color='red', edgecolor='black')
        ...
        axes[1, 2].scatter(df['sequence_length'], df['computation_time'], alpha=0.7, color='brown')
